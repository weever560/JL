...
static u8 is_app_spple_active = 0; //允许进入低功耗
...
void led_on(void)
{
    is_app_spple_active = 1;//不允许进入低功耗
    gpio_set_output_value(IO_PORTA_01,1);
}
void led_off(void)
{
    is_app_spple_active = 0;//允许进入低功耗
...
static void spple_key_event_handler(struct sys_event *event)
{
    ...
    if (event->arg == (void *)DEVICE_EVENT_FROM_KEY) {
        ...
        if (event_type == KEY_EVENT_CLICK && key_value == TCFG_ADKEY_VALUE2) {
            {   log_info(">>>hold key2:chage case to idle\n");
                trans_disconnect();
            }
